...
const NRW = () => {
    ...
    if (!initialized || !countries) { return <Loader />; }

    const germany = countries.find(c => c.id === 2); // Германия
    const nrw = germany?.germanStates.items.find(state => state.id === 10);
    ...
    const crumbs = [
        ...
        {
            label: lang === 'ru' ? 'Германия' : lang === 'de' ? 'Deutschland' : 'Німеччина',
            path: '/germany'
        },
        {
            label: nrw?.name
                        ...
                        <ul className="nrw__sidebar-list">
                            {regionBlock.items.map((reg) => (
                                <li key={reg.id} className="nrw__sidebar-item">
                                    {reg.hasInfo ? (
                                        <Link to={reg.path} className="nrw__sidebar-link">
                                            {reg.name}
                                        </Link>
                                    ) : (
            ...
            <div className="nrw__content">
                ...
                <div className="nrw__history">
                    ...
                    {nrw.briefHistory.items.map((item, index) => (
                        <p key={index} className="nrw__history-text">
                            {item.text}
                        </p>
                    ))}
                </div>
